...
public class Demo {
	...
	public static void demoXingeSimple() {
		...
		System.out.println(
				XingeApp.pushTokenIos(000L, "secretKey", "测试", "token", XingeApp.IOSENV_DEV));
		System.out.println(
				XingeApp.pushAccountIos(000L, "secretKey", "测试", "account", XingeApp.IOSENV_DEV));
		System.out.println(XingeApp.pushAllIos(000L, "secretKey", "测试", XingeApp.IOSENV_DEV));
		System.out
				.println(XingeApp.pushTagIos(000L, "secretKey", "测试", "tag", XingeApp.IOSENV_DEV));
	}
	...
	protected JSONObject demoPushAccountListMultiple() {
		...
		try {
			if (ret.getInt("ret_code") != 0) {
				...
				// ...
				result.accumulate("all", xinge.pushAccountListMultiple(
						ret.getJSONObject("result").getLong("push_id"), accountList1));
				...
				// ...
				result.accumulate("all", xinge.pushAccountListMultiple(
						ret.getJSONObject("result").getLong("push_id"), accountList2));
				return result;
			}
	...
	protected JSONObject demoPushDeviceListMultiple() {
		...
		try {
			if (ret.getInt("ret_code") != 0) {
				...
				// ...
				result.accumulate("all",
						xinge.pushDeviceListMultiple(ret.getJSONObject("result").getLong("push_id"),
								deviceList1));
	...
	protected void buildMesssges() {
		...
		action.setIntent(
				"intent:10086#Intent;scheme=tel;action=android.intent.action.DIAL;S.key=value;end");
